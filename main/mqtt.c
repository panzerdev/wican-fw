...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base,
  ...
  case MQTT_EVENT_CONNECTED:
     ...
     *
     * This character array is used to format and temporarily hold status
     * messages that will be published to MQTT topics. The buffer size of 128
     * bytes should accommodate typical status message content including device
     * state, error codes, and diagnostic information.
     */
    char status_msg[128];
...
static char *mqtt_load_cert(const char *filename) {
  FILE *f = fopen(filename, "r");
  if (f == NULL) {
    ESP_LOGE(TAG, "Failed to open cert file: %s", filename);
    return NULL;
  }
  fseek(f, 0, SEEK_END);
  long length = ftell(f);
...
void mqtt_init(char *id, uint8_t connected_led, QueueHandle_t *xtx_queue) {
  ...
  if (config_server_mqtt_secure_en_config() == 1) {
    ESP_LOGI(TAG, "MQTT Secure Enabled");
    mqtt_cfg.broker.address.transport =
        MQTT_TRANSPORT_OVER_SSL; // Force SSL transport

    static char *ca_cert = NULL;
    static char *client_cert = NULL;
    static char *client_key = NULL;

    // Free if previously allocated (though normally init is called once per
    // boot)
    if (ca_cert)
      free(ca_cert);
    if (client_cert)
    ...
    ca_cert = mqtt_load_cert(FS_MOUNT_POINT "/ca_cert.pem");
    client_cert = mqtt_load_cert(FS_MOUNT_POINT "/client_cert.pem");
    client_key = mqtt_load_cert(FS_MOUNT_POINT "/client_key.pem");

    if (ca_cert) {
      mqtt_cfg.broker.verification.certificate = ca_cert;
      ESP_LOGI(TAG, "Loaded CA Cert");
    }
    if (client_cert) {
      mqtt_cfg.credentials.authentication.certificate = client_cert;
      ESP_LOGI(TAG, "Loaded Client Cert");
    }
    if (client_key) {
      mqtt_cfg.credentials.authentication.key = client_key;
      ESP_LOGI(TAG, "Loaded Client Key");
    }
  }
