...
# Announce starting
echo "Starting deploy: local git short hash ${LOCAL_GIT_SHORT}" >&2
...
echo "Pinging ${TARGET_HOST}..." >&2
echo "Checking HTTP on http://${TARGET_HOST}/ ..." >&2
...
echo "Fetching remote git_version..." >&2
...
echo "Building firmware in Docker..." >&2
...
if [ -f "$FW_PATH" ]; then
	echo "Uploading firmware ${FW_PATH} to http://${TARGET_HOST}/upload/ota.bin ..." >&2
